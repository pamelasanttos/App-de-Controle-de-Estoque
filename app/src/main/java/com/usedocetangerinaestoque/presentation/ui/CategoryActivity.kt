...
import com.usedocetangerinaestoque.presentation.viewmodel.CategoryViewModel
...
class CategoryActivity : BaseDrawerActivity() {
    ...
    private fun setupAddButton() {
        findViewById<ImageButton>(R.id.btnAddItem).setOnClickListener {
            showInputDialog("Nova categoria:") { newCategory ->
    ...
    private fun observeViewModel() {
        viewModel.filteredCategories.observe(this) { list ->
            containerItems.removeAllViews()

            if (list.isEmpty()) {
                showToast("Nenhuma categoria encontrada para a busca.")
                return@observe
            }
    ...
    private fun createCategoryCard(category: Category): View {
        ...
        // quantity.text = "Itens: ${category.totalItems}"

        btnEdit.setOnClickListener {
            showInputDialog("Editar categoria:") { newName ->
                viewModel.updateCategory(Category(id = category.id, name = newName))
            }
        }
    ...
    private fun showDeleteConfirmation(category: Category) {
        AlertDialog.Builder(this)
            .setTitle("Excluir \"${category.name}\"?")
